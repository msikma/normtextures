 ...
 */
const generateMapOverview = (mapData, headerItems, descriptions, basedirs = []) => {
  const buffer = []
  buffer.push('<tbody>')
  buffer.push('<tr>')
  ...
  for (const file of mapData.files) {
    ...
    buffer.push(`<td><a href="${getFileLink(file.filename, mapData.name, basedirs)}">${file.name}</a></td>`)
    buffer.push(`<td>${file.description}</td>`)
    buffer.push(`<td class="c1"><div class="c2">${getFileImage(file.filename, mapData.name, basedirs)}</div></td>`)
 ...
 */
const generateOverview = (allMapsData, descriptions, basedirs = []) => {
  const headers = ['#', 'Name', 'Description', 'Image', 'Kind', 'Size', 'Unknown', 'Flags']
  const buffer = []
  buffer.push('<table class="table overview" border="2">')
  buffer.push(...(allMapsData.map(mapData => generateMapOverview(mapData, headers, descriptions, basedirs))))
  buffer.push('</table>')
  return buffer.join('\n')
}
