 ...
 */
const getFileLink = (link, name, basedirs = [], pathPrefix = './') => {
  return `${pathPrefix}${path.join(...basedirs, name, link)}`
}

/**
 * Returns an image tag for a texture.
 */
const getFileImage = (link, name, basedirs = [], pathPrefix = './') => {
  const filepath = getFileLink(link, name, basedirs, pathPrefix)
